...
//Version      2015-06-20  z
    //修改_nPtrData从int到INT64，避免编译警告
/*
...
#define DECLARE_EVENT(access,f,p,v) \
	...
	{\
		...
		{\
			...
			pNode->_pThis = pThis;\
			pNode->_nPtrData = (INT64)pThis + pThis->_xEventReceiver.nOffset;\
			pNode->_pFun = pFun;\
...
#define DECLARE_EVENT_RECEIVER2(exdesc, classname) \
	public:\
	struct exdesc _event_Receiver\
	{\
		_event_Receiver(){memset(data,'#',16);nOffset=offsetof(classname,_xEventReceiver);}\
		~_event_Receiver(){memset(data,0,16);}\
		BYTE data[16];\
